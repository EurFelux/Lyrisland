...
- **靈動島形態** — 緊湊、展開、完整三種模式，點按切換，流暢動畫過渡
- **即時同步** — 歌詞逐行高亮，與播放進度精準對齊
- **多歌詞源** — 自動從多個來源尋找歌詞，找不到時智慧回退
- **無需登入** — 直接讀取本機 Spotify 客戶端狀態，無需授權 Spotify 帳號
- **輕量常駐** — 僅在選單列執行，不佔用 Dock 列，低資源佔用
- **手動微調** — 支援歌詞偏移量調整（±0.5s），適配不同歌詞源的時間差
...
- [Lyricify Lyrics Helper](https://github.com/WXRIW/Lyricify-Lyrics-Helper) — Musixmatch 歌詞 API 參考
